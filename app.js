...
function App() {
  ...
  return <>
    ...
    <main className="main-content" id="dashboard">
      ...
      <section className="filter-bar"><div className="filter-heading"><span className="filter-icon">≡</span><strong>Filtros</strong><button className="clear-filters" onClick={() => setFilters({ period: "Últimos 30 dias", status: "Todos", client: "Todos" })}>Limpar filtros</button></div><label>Período<select value={filters.period} onChange={(event) => setFilters({ ...filters, period: event.target.value })}><option>Últimos 30 dias</option><option>Últimos 7 dias</option><option>Este mês</option></select></label><label>Status<select value={filters.status} onChange={(event) => setFilters({ ...filters, status: event.target.value })}>{statusOptions.map((option) => <option key={option}>{option}</option>)}</select></label><label>Cliente<select value={filters.client} onChange={(event) => setFilters({ ...filters, client: event.target.value })}>{options.clients.map((option) => <option key={option}>{option}</option>)}</select></label></section>

      <section className="kpi-grid"><KpiCard label="Total de entregas" value={metrics.total} detail="no período selecionado" tone="teal" icon="↗" /><KpiCard label="Concluídas" value={metrics.completed} detail={`${metrics.total ? Math.round(metrics.completed / metrics.total * 100) : 0}% do total`} tone="green" icon="✓" /><KpiCard label="Em andamento" value={metrics.pending} detail="aguardando conclusão" tone="amber" icon="◷" /><KpiCard label="Atrasadas" value={metrics.late} detail="requerem atenção" tone="red" icon="!" /><KpiCard label="No prazo" value={`${metrics.punctuality}%`} detail="das concluídas" tone="blue" icon="⌁" /></section>

      <section className="analytics-grid"><article className="panel status-panel"><div className="panel-header"><div><h2>Distribuição por status</h2><p>Visão geral das entregas filtradas</p></div><button className="dots">•••</button></div><div className="status-content"><div className="donut" style={{ background: `conic-gradient(${palette[statusCounts[0].status]} 0 ${metrics.total ? statusCounts[0].count / metrics.total * 100 : 0}%, ${palette[statusCounts[1].status]} 0 ${metrics.total ? (statusCounts[0].count + statusCounts[1].count) / metrics.total * 100 : 0}%, ${palette[statusCounts[2].status]} 0 ${metrics.total ? (statusCounts[0].count + statusCounts[1].count + statusCounts[2].count) / metrics.total * 100 : 0}%, ${palette[statusCounts[3].status]} 0 100%)` }}><div><strong>{metrics.total}</strong><span>entregas</span></div></div><div className="legend">{statusCounts.map((item) => <div className="legend-row" key={item.status}><span className="legend-color" style={{ background: palette[item.status] }} /> <span>{item.status}</span><strong>{item.count}</strong><small>{metrics.total ? Math.round(item.count / metrics.total * 100) : 0}%</small></div>)}</div></div></article></section>
